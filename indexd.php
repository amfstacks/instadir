<?php
/**
 * STRUCTIFY - v5 (The Bulletproof Cleaner)
 * Supports CI4, Laravel, React, AND aggressive comment stripping (#, ()).
 */
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['tree_text'])) {
    ...
    foreach ($lines as $line) {
        // ULTIMATE CLEANER: Strip anything after #, (, or //
        ...
        $prefix = $matches[1];
        // Aggressively trim the name, remove trailing slashes, and trim again just in case
        $name = trim($matches[2]);
        $name = rtrim($name, '/');
        $name = trim($name);
...
<body class="bg-slate-50 min-h-screen text-slate-800 font-sans">
                ...
                <div class="p-6 lg:p-8 border-b lg:border-b-0 lg:border-r border-slate-200 bg-slate-50">
                    <form method="POST" action="">
                        ...
                        <div class="mt-6 flex justify-end">
                            <button type="submit" class="w-full sm:w-auto bg-indigo-600 hover:bg-indigo-700 text-white font-bold py-3 px-8 rounded-lg shadow-md transition-all duration-200 transform hover:-translate-y-0.5">
                                Build Architecture
                            </button>
                        </div>
                    </form>
    ...
    <script>
        ...
        function updatePreview() {
            ...
            text.split('\n').forEach(line => {
                // ULTIMATE CLEANER (JS Version): Strip #, (, and trailing spaces
                let cleaned = line.replace(/\s*[\(#].*$/, '').trimEnd();
                
                const match = cleaned.match(/^([│├└─\s ]*)(.*)$/);
                if (!match) return;

                // Aggressive trim and slash removal
